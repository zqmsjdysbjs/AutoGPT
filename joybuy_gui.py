...
def build_dictionary_template() -> str:
    head = "产品描述，Product description,1.Produktbeschreibung, Produktbeschreibung, Kurzbeschreibung, , "
    tail = ", ".join([f"{i}.空格" for i in range(2, 16)])
    return head + tail


def copy_ai_instruction_with_detail():
    text = detail_text.get("1.0", tk.END).strip()
    if not text:
        status_label.config(text="❌ 错误：请先在商详输入框中输入内容")
        return
    final_text = AI_DE_POLISH_PROMPT + "\n\n" + text
    try:
        pyperclip.copy(final_text)
        status_label.config(text="✅ 已复制：AI审校提示词 + 商详 文本到剪贴板")
    except Exception as e:
        status_label.config(text=f"❌ 复制失败：{str(e)}")


def clear_detail_and_copy_dictionary():
    try:
        detail_text.delete("1.0", tk.END)
    except Exception:
        pass
    try:
        dict_text = build_dictionary_template()
        pyperclip.copy(dict_text)
        status_label.config(text="✅ 已清空商详并复制词典模板到剪贴板")
    except Exception as e:
        status_label.config(text=f"❌ 复制失败：{str(e)}")
...
root = tk.Tk()
try:
    root.iconbitmap(ICON_PATH)  # 若有图标则设置，Linux下ico可能不生效
except Exception:
    pass
root.title("德国站商详批量打开和joybuy前端页面批量审阅工具")
root.geometry("780x680")  # 调整高度，为新增输入框留空间
...
style.configure("Copy.TButton", foreground="#ffffff", background="#2d9cdb")  # 复制按钮（蓝色）
style.configure("Dict.TButton", foreground="#ffffff", background="#a333c8")  # 词典按钮（紫色）
# 按钮悬停效果
...
style.map("Copy.TButton", background=[("active", "#2384b8")])
style.map("Dict.TButton", background=[("active", "#7e2da4")])
...
# 新增：商详输入框
_detail_label_text = "商详文本（输入后可与AI提示词拼接复制）："
detail_input_prompt = ttk.Label(sku_input_frame, text=_detail_label_text)
detail_input_prompt.pack(anchor="w", pady=(5, 3))
...
detail_text.pack(fill=tk.BOTH, expand=True, pady=(0, 5))
...
# 新增按钮4：复制AI提示词 + 商详
copy_ai_btn = ttk.Button(
    btn_frame,
    text="4. 复制AI审校提示词 + 商详 到剪贴板",
    command=copy_ai_instruction_with_detail,
    style="Copy.TButton"
)
copy_ai_btn.pack(pady=2, fill=tk.X)

# 新增按钮5：清空商详并复制词典模板
copy_dict_btn = ttk.Button(
    btn_frame,
    text="5. 清空商详并复制词典模板 到剪贴板",
    command=clear_detail_and_copy_dictionary,
    style="Dict.TButton"
)
copy_dict_btn.pack(pady=2, fill=tk.X)
