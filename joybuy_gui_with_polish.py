...
def handle_concat_copy():
    """将商详与指令拼接并复制到剪贴板"""
    content = polish_text.get("1.0", tk.END).strip()
    if not content:
        status_label.config(text="❌ 错误：请先在商详输入框中键入或粘贴内容")
        return
    combined_text = f"{content}\n\n{AI_PROMPT_DE_INSTRUCTIONS}"
    try:
        pyperclip.copy(combined_text)
        status_label.config(text=f"✅ 已复制到剪贴板（{len(combined_text)}字符）")
    except Exception as e:
        status_label.config(text=f"❌ 复制失败：{str(e)}")


def build_dictionary_template():
    """构建词典模板字符串"""
    header = "产品描述，Product description"
    first_line = "1. Produktbeschreibung, Produktbeschreibung, Kurzbeschreibung"
    other_lines = [f"{i}. 空格" for i in range(2, 16)]
    return "\n".join([header, first_line] + other_lines)


def handle_generate_dictionary_template():
    """清空商详输入框，生成词典模板并复制到剪贴板"""
    try:
        polish_text.delete("1.0", tk.END)
    except Exception:
        pass
    dictionary_text = build_dictionary_template()
    try:
        pyperclip.copy(dictionary_text)
        status_label.config(text="✅ 已生成词典模板，输入框已清空并复制到剪贴板")
    except Exception as e:
        status_label.config(text=f"❌ 复制失败：{str(e)}")
...
root.geometry("780x720")  # 稍增高度以容纳新功能
...
# 输入容器内部Grid：左3右1分栏
input_container_frame.grid_rowconfigure(0, weight=3)  # 批量输入框占更多空间
input_container_frame.grid_rowconfigure(1, weight=1)  # 单个输入框占较少空间
...
# 在“左侧：SKU输入区域”中，添加以下代码（放在批量SPU分类输入框下方）：
...
# 新增：商详润色助手（位于输入容器下方，独占整行）
input_container_frame.grid_rowconfigure(2, weight=2)
polish_frame = ttk.Frame(input_container_frame)
polish_frame.grid(row=2, column=0, columnspan=2, sticky="nsew", pady=(10, 0))

polish_title = ttk.Label(
    polish_frame,
    text="✍️ 商详润色助手（AI喂入模板）",
    font=("微软雅黑", 10, "bold"),
    foreground="#000000",
    background="#f5f5f5"
)
polish_title.pack(anchor="w", pady=(0, 6))

polish_text = scrolledtext.ScrolledText(
    polish_frame,
    width=70,
    height=8,
    font=("微软雅黑", 10),
...
polish_text.pack(fill=tk.BOTH, expand=True, pady=(0, 6))

polish_btns_frame = ttk.Frame(polish_frame)
polish_btns_frame.pack(fill=tk.X)

concat_copy_btn = ttk.Button(
    polish_btns_frame,
    text="A. 合并提示并复制（商详 + 指令）",
    command=handle_concat_copy,
    style="Normal.TButton"
)
concat_copy_btn.pack(side=tk.LEFT, padx=(0, 6))

dict_btn = ttk.Button(
    polish_btns_frame,
    text="B. 生成词典模板（清空并复制）",
    command=handle_generate_dictionary_template,
    style="Normal.TButton"
)
dict_btn.pack(side=tk.LEFT, padx=(0, 6))
